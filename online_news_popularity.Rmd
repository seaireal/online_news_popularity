...
# auc
lda_roc <-roc(test_ranks,lda_pred$posterior.1, levels = c(0, 1), direction = "<")
auc(lda_roc)
```
...
# auc
qda_roc <- roc(test_ranks,qda_pred$posterior.1, levels = c(0, 1), direction = "<")
auc(qda_roc)
```
...
#knn_mod = train(ranks ~ . -shares, data = df_train, method = "knn", preProcess = c("center","scale"), tuneLength = 17)
#knn_pred = predict(knn_mod,newdata = df_test, type = "prob")
