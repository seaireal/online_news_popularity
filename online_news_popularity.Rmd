...
plot(knn_test_error, xlab=NA, ylab="test error rate", col="red", cex=2, lwd=2, cex.lab=2)
abline(v=17, lty=2)
title("KNN", cex.main=2)
plot(knn_test_sen, xlab=NA, ylab="sensitivity", col="blue", cex=2, lwd=2, cex.lab=2)
abline(v=15, lty=2)
plot(knn_test_spe, xlab="K", ylab="specificity", col="purple", cex=2, lwd=2, cex.lab=2)
abline(v=17, lty=2)
```
...
rfmod_impgg = rfmod_imp %>% ggplot(aes(reorder(predictor, MeanDecreaseAccuracy), MeanDecreaseAccuracy, fill=predictor)) + 
                              geom_col(show.legend=FALSE) + xlab(NULL) + ylab("mean decrease in accuracy") + coord_flip() +
                              scale_fill_hue(l=50) + theme_classic() +
...
ptm = proc.time()
set.seed(1)
bmod = gbm(ranks~.-shares, df_train, distribution="bernoulli", n.trees=500, interaction.depth=4, shrinkage=0.2, verbose=F)
time_spent = proc.time() - ptm
paste("Processing time:", round(time_spent[3]))
```
