...
df <- df %>% select(-url, -timedelta) %>% 
  ...
  ) %>% select(-starts_with("data_channel_is"), -starts_with("weekday_is"), -weekday)
...
lmod_subset = regsubsets(lmod_formula, data=df, method="forward", nvmax=50)
...
points(11,lmod_subset_summary$bic[11],col="red",cex=2,pch=20)
plot(lmod_subset_summary$adjr2, xlab = "Subset Size", ylab = "Adjusted R2", pch = 20, type = "l",
     main = "adj_R2 of the Model")
points(41,lmod_subset_summary$adjr2[41],col="red",cex=2,pch=20)
