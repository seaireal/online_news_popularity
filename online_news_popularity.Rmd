...
anova(gam.fit0, gam.fit2, gam.fit1, test="F")
# gam.fit2 is the best among all three

gam.pred = predict(gam.fit2, df_test)
...
ldamod_pred_auc = prediction(lda_preds$x, test_ranks)
performance(ldamod_pred_auc, "auc")@y.values[[1]]
```
...
qda_mod = qda(formula_lg, data = df_train,type="prob")
...
# auc
qdaroc <-roc(test_ranks,as.numeric(qda_pred$posterior[,2]), levels = c(0, 1), direction = "<")
auc(qdaroc)

qdamod_pred_auc = prediction(qda_pred$posterior[,2], test_ranks)
performance(qdamod_pred_auc, "auc")@y.values[[1]]
```
