...
# num_hrefs seams linear, apply the same methos based on gam.fit2 and choose gam.fit4
gam.fit3 = gam(log(shares) ~s(num_imgs, df = 2) + average_token_length + s(num_keywords, df = 2) + s(kw_avg_min, df = 2) + s(kw_avg_max, df = 2) + s(kw_avg_avg, df = 2) + s(self_reference_avg_sharess, df = 2) + is_weekend + LDA_01 + LDA_02 + LDA_03 + s(global_subjectivity, df=2) + s(title_sentiment_polarity, df = 2), data = df_train) # without num_hrefs
gam.fit4 = gam(log(shares) ~ num_hrefs + s(num_imgs, df = 2) + average_token_length + s(num_keywords, df = 2) + s(kw_avg_min, df = 2) + s(kw_avg_max, df = 2) + s(kw_avg_avg, df = 2) + s(self_reference_avg_sharess, df = 2) + is_weekend + LDA_01 + LDA_02 + LDA_03 + s(global_subjectivity, df=2) + s(title_sentiment_polarity, df = 2), data = df_train) # with num_hrefs as linear
# anova(gam.fit3, gam.fit4, gam.fit2, test="F")
# par(mfrow = c(2, 2))
# plot(gam.fit4, se = T, col = "blue")

# global_subjectivity seams linear, apply the same methos based on gam.fit4 and choose gam.fit6
gam.fit5 = gam(log(shares) ~ num_hrefs + s(num_imgs, df = 2) + average_token_length + s(num_keywords, df = 2) + s(kw_avg_min, df = 2) + s(kw_avg_max, df = 2) + s(kw_avg_avg, df = 2) + s(self_reference_avg_sharess, df = 2) + is_weekend + LDA_01 + LDA_02 + LDA_03 + s(title_sentiment_polarity, df = 2), data = df_train) # without global_subjectivity
gam.fit6 = gam(log(shares) ~ num_hrefs + s(num_imgs, df = 2) + average_token_length + s(num_keywords, df = 2) + s(kw_avg_min, df = 2) + s(kw_avg_max, df = 2) + s(kw_avg_avg, df = 2) + s(self_reference_avg_sharess, df = 2) + is_weekend + LDA_01 + LDA_02 + LDA_03 + global_subjectivity + s(title_sentiment_polarity, df = 2), data = df_train) # with global_subjectivity as linear
...
#coef(lda_mod) %>% dim()
lda_preds = predict(lda_mod, df_test)
...
lda_pred_auc = prediction(lda_pred, test_ranks)
performance(lda_pred_auc, "auc")@y.values[[1]]

ldaroc <-roc(test_ranks,as.numeric(lda_preds$x), levels = c(0, 1), direction = "<")
auc(ldaroc)

ldaroc <-roc(test_ranks,as.numeric(lda_ranks), levels = c(0, 1), direction = "<")
auc(ldaroc)
```
...
rfmod_roc = roc(test_ranks, rfmod_pred[,2], levels = c(0, 1), direction = "<")
auc(rfmod_roc)
